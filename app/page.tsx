  ...
  return (
    <main className="relative min-h-screen overflow-x-hidden">
      ...
      <div className="relative mx-auto flex w-full max-w-5xl flex-col gap-6 px-4 py-10 sm:px-6 lg:px-8">
        <header className="relative overflow-hidden rounded-xl border-2 border-app-line bg-app-card p-6 glow-border-cyan">
        ...
        {isAuthed && envHealth && !envHealth.database && (
          <p className="rounded-lg border border-amber-500/30 bg-amber-950/30 px-4 py-2 text-center text-sm text-amber-200/90">
            Set <code className="text-app-in/90">DATABASE_URL</code> in <code className="text-app-in/90">.env</code> and run{" "}
            <code className="text-app-in/90">npx prisma db push</code> so watches can be saved. See{" "}
            <code className="text-app-in/90">SETUP_OAUTH.md</code>.
          </p>
        )}

        {!isAuthed ? (
          <div className="rounded-xl border-2 border-dashed border-app-dash/80 bg-app-body/80 p-10 text-center">
            <p className="text-lg text-app-in/90">Sign in with Google to save your watches in the cloud.</p>
            <p className="mt-2 text-sm text-app-muted">Your sync history and graphs persist across devices.</p>
          </div>
        ) : (
          <>
        <section className="grid gap-4 sm:grid-cols-[1fr_280px]">
          ...
          <div className="rounded-xl border-2 border-app-line bg-app-card p-4 text-xs text-app-muted">
            <p className="mb-1 flex items-center gap-2 text-[10px] uppercase tracking-widest text-app-a1/60">
              <Crosshair className="h-3 w-3" /> how it works
            </p>
            <ol className="list-decimal space-y-1 pl-4 text-[11px] leading-relaxed text-app-muted/90">
        ...
        {watches.length === 0 ? (
          <div className="rounded-xl border border-dashed border-app-dash/40 p-12 text-center text-app-muted text-sm">
            No watches yet. Add one above.
          </div>
        ) : (
          <div className="space-y-4">
            <div className="space-y-3">
              <p className="text-[10px] uppercase tracking-[0.28em] text-app-muted">Select watch</p>
                  ...
                  {watches.map((w) => (
                    <motion.button
                      ...
                      className={cn(
                        ...
                        w.id === selectedId
                          ? "border-app-a1/60 bg-app-a1/10 text-app-in"
                          : "border-app-line/80 bg-app-body/50 text-app-muted hover:border-app-line"
                      )}
                    >
    ...
    <>
      <div className="space-y-5 lg:col-span-1">
        <motion.div
          ...
          className="rounded-xl border-2 border-app-line bg-app-card p-5"
        >
          <div className="mb-4 flex items-start justify-between gap-2">
            <div>
              <h2 className="text-lg font-bold uppercase tracking-widest text-app-in">
                {watch.name}
              </h2>
          ...
          <p className="mb-3 text-xs text-app-muted">
            Match the <span className="text-app-a2">right</span> dial to your physical watch. The
            app displays atomic time on the <span className="text-app-a1">left</span> (browser clock).
            <span className="text-app-a1/90"> +</span> = your watch is{" "}
            <span className="text-app-a1">fast</span> (ahead),<span className="text-app-a2/90"> −</span>{" "}
            = <span className="text-app-a2">slow</span> (behind). Record stores this difference
            in seconds.
          </p>
          ...
          <div className="flex flex-col items-center gap-2 py-2">
            <p className="text-[10px] uppercase tracking-[0.2em] text-app-muted">drift to record</p>
            <div className="app-offset-hero font-mono text-4xl font-bold tabular-nums text-app-in sm:text-5xl">
              {offset > 0 ? "+" : ""}
              {offset}
              <span className="text-xl text-app-muted sm:text-2xl">s</span>
            </div>
            <div className="flex flex-wrap items-center justify-center gap-1.5">
              {[
                ...
                <button
                  ...
                  type="button"
                  onClick={() => nudge(b.d)}
                  className="flex h-9 min-w-[44px] items-center justify-center rounded border border-app-btn-n bg-app-a1/5 px-1.5 text-[10px] font-bold tabular-nums text-app-in/90 hover:border-app-a1/60 hover:bg-app-a1/10 sm:text-xs"
                >
                  {b.label}
                </button>
              ))}
            ...
            <div className="flex gap-2">
              <button
                type="button"
                onClick={() => onOffset(0)}
                className="rounded-lg border border-app-line/80 px-3 py-1.5 text-[10px] uppercase tracking-widest text-app-muted hover:text-app-fg/90"
              >
                Reset 0
              </button>
            </div>
            <button
              ...
              className="app-record-cta flex w-full max-w-sm items-center justify-center gap-2 rounded-xl border-2 border-app-record-b bg-app-a2/10 py-3 text-sm font-bold uppercase tracking-[0.2em] text-app-record-fg transition hover:brightness-110"
            >
        ...
        {hint && <p className="text-center text-[10px] text-app-muted">{hint}</p>}
      </div>

      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <p className="text-[10px] uppercase tracking-[0.28em] text-app-muted">Offset over time</p>
        </div>
        <div className="rounded-xl border-2 border-app-line bg-app-card p-4 glow-border-cyan">
          <DriftChart syncs={watch.syncs} />
        </div>
        <p className="px-1 text-[10px] text-app-muted">
...
function StatPill({
  ...
  return (
    <div className="rounded-lg border border-app-line/70 bg-app-body/40 px-3 py-2.5">
      <div className="mb-0.5 flex items-center justify-between text-[9px] uppercase tracking-widest text-app-muted">
        <span>{label}</span>
        {icon}
      </div>
      <div className="truncate text-sm font-bold text-app-in/90">{value}</div>
      {sub && <div className="mt-0.5 text-[9px] text-app-muted/80">{sub}</div>}
    </div>
  );
}
